...
export const blogPosts: BlogPost[] = [
  {
    ...
    title: 'Microsoft Store 自动化上架：MSIX 打包、Submission API 与发布脚本',
    excerpt: '围绕 MSIX Packaging Tool、MSIX Toolkit 和 Microsoft Store Submission API，整理从打包、签名、ZIP 上传到提交审核的自动化发布流程，说明哪些步骤能脚本化，哪些仍需 Partner Center 一次性初始化。',
    date: '2026-07-14',
    ...
    content: [
      ...
      {
        type: 'paragraph',
        text: 'Microsoft Store 的自动化上架，目标是让日常版本发布不再反复打开 Partner Center 页面。它不是绕过开发者账号、商店审核或合规资料，而是在完成一次性配置后，用脚本创建提交、更新包体和元信息、上传文件、提交审核并轮询状态。',
      },
      {
        type: 'quote',
        text: '最稳的理解是：Partner Center 负责账号、产品和审核规则；脚本负责把重复发布动作变成可复用的流水线。',
      },
      {
        type: 'heading',
        text: '工具边界',
      },
      {
        type: 'list',
        items: [
          'MSIX Packaging Tool：Microsoft Store 提供的打包工具，可把现有 MSI、EXE、App-V 或桌面应用重新打包为 MSIX，也支持通过命令行模板做批量转换。',
          'MSIX Toolkit Scripts：微软开源的脚本集合，适合做 MSIX 包管理、签名、Publisher 修改、App Installer 等辅助动作，但它不直接替代商店提交。',
          'Microsoft Store Submission API：负责创建提交、更新提交 JSON、上传包体 ZIP、commit 提交并查询审核状态，是“不登录开发者后台发布更新”的核心。',
          'StoreBroker：微软提供的开源 PowerShell 模块，封装了 Store Submission API，可作为直接调用 API 之外的命令行方案或参考实现。',
        ],
      },
      {
        type: 'heading',
      ...
      {
        type: 'paragraph',
        text: '如果项目本身能直接产出 MSIX、MSIXBundle 或 MSIXUpload，优先使用项目自身构建链路。如果是传统安装包转换，可以先在 MSIX Packaging Tool 图形界面生成 conversion template，再在发布脚本中复用模板。',
      },
      {
        type: 'code',
        language: 'powershell',
        code: [
          ...
          '  --template C:\\release\\ConversionTemplate.xml `',
          '  --virtualMachinePassword $env:MSIX_VM_PASSWORD `',
          '  -v',
        ].join('\n'),
      },
      {
        type: 'list',
        items: [
          '转换环境尽量使用干净虚拟机，减少用户缓存、临时文件、无关注册表项被捕获进包里。',
          'Package/Identity/Name、Publisher、Version 必须与 Partner Center 中的包身份链路一致，版本号每次发布都要递增。',
          '打包后要先本地安装测试，覆盖安装、启动、权限弹窗、卸载、更新和主流程回归。',
          '如果包内 Publisher 需要调整，可参考 MSIX Toolkit Scripts 中的 ModifyPackagePublisher 脚本思路，但生产发布前一定要重新验证签名和身份匹配。',
        ],
      },
      {
        type: 'heading',
      ...
      {
        type: 'heading',
        text: 'PowerShell 脚本骨架',
      },
      {
        type: 'paragraph',
        text: '下面是适合沉淀到发布仓库的脚本骨架。真实项目中需要按应用类型补齐包体结构、截图路径、商店列表 JSON 和错误处理，但主流程就是获取 token、创建提交、更新包信息、上传 ZIP、commit、轮询状态。',
      },
      {
        type: 'code',
        language: 'powershell',
        code: [
          ...
          '$baseUrl = "https://manage.devcenter.microsoft.com/v1.0/my/applications/$ApplicationId"',
          '',
          '$submission = Invoke-RestMethod -Method Post -Uri "$baseUrl/submissions" -Headers $headers',
          ...
          ')',
          '',
          '$body = $submission | ConvertTo-Json -Depth 50',
          'Invoke-RestMethod -Method Put `',
          '  -Uri "$baseUrl/submissions/$($submission.id)" `',
          '  -Headers $headers `',
          ...
          '  -Uri "$baseUrl/submissions/$($submission.id)/commit" `',
          '  -Headers $headers',
          '',
          'do {',
          '  Start-Sleep -Seconds 30',
          '  $status = Invoke-RestMethod -Method Get `',
          '    -Uri "$baseUrl/submissions/$($submission.id)/status" `',
          '    -Headers $headers',
          '  Write-Host "submission status: $($status.status)"',
          '} while ($status.status -in @("CommitStarted", "PreProcessing"))',
        ].join('\n'),
      },
      {
        type: 'heading',
      ...
      },
      {
        ...
        items: [
          ...
          '如果只是上传 MSI/EXE 更新，也可以参考 microsoft/store-submission GitHub Action；如果是 MSIX/UWP 类型，优先对照对应的 Store Submission API 或 StoreBroker 实现。',
        ],
      },
      {
        type: 'heading',
      ...
      },
      {
        ...
        items: [
          ...
          '自动化脚本已在测试提交或 flight 中验证过，确认不会覆盖错误市场、价格、描述或包体。',
        ],
      },
      {
        type: 'heading',
      ...
      },
      {
        ...
        items: [
          ...
          '脚本创建提交后后台无法继续改：同一个 API submission 不要再用 Partner Center 页面混改，必要时删除草稿后重新创建。',
          '脚本突然无法发布：检查 Client Secret 是否过期、Entra 应用是否仍关联 Partner Center、账号角色是否有提交权限。',
        ],
      },
      {
        type: 'heading',
      ...
      },
      {
        type: 'list',
        items: [
          'MSIX Packaging Tool：https://apps.microsoft.com/detail/9n5lw3jbcxkf?hl=zh-CN&gl=SG',
          'MSIX Toolkit Scripts：https://github.com/microsoft/MSIX-Toolkit/tree/master/Scripts',
          'MSIX Packaging Tool 命令行文档：https://learn.microsoft.com/en-us/windows/msix/packaging-tool/package-conversion-command-line',
          ...
          'StoreBroker PowerShell 模块：https://github.com/microsoft/StoreBroker',
        ],
      },
      {
        type: 'heading',
      ...
      {
        type: 'paragraph',
        text: 'Microsoft Store 自动化发布的关键不是单独写一个上传脚本，而是把包身份、打包模板、商店元信息、API 提交、状态轮询和失败排查连成一条稳定链路。前期把一次性后台配置补齐，后续版本就可以用脚本把重复工作压到最低。',
      },
    ],
  },
  {
